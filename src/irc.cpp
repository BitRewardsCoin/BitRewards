...
void ThreadIRCSeed()
{
    ...
    RenameThread("bitrewards-ircseed");
...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #bitrewardscoinTEST\r");
            Send(hSocket, "WHO #bitrewardscoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #bitrewardscoin\n");
            // randomly join #bitrewardscoin00-#bitrewardscoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #bitrewardscoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #bitrewardscoin%02d\r", channel_number).c_str());
        };
