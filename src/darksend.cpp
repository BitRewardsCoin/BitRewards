...
bool CDarkSendPool::DoAutomaticDenominating(bool fDryRun, bool ready)
{
    ...
    int64_t nBalanceNeedsAnonymized = nAnonymizeBitRewardsAmount*COIN - pwalletMain->GetAnonymizedBalance();
...
//        }
    ...
    if(!sessionFoundMasternode){
        ...
        double fBitRewardsSubmitted = (sessionTotalValue / CENT);
        LogPrintf("Submitting Darksend for %f XBR CENT - sessionTotalValue %d\n", fBitRewardsSubmitted, sessionTotalValue);
        ...
        if(c % 60*(nLiquidityProvider+1) == 0){
            if(nLiquidityProvider!=0){
                ...
                if(nRand == 50+nLiquidityProvider && pwalletMain->GetAverageAnonymizedRounds() > 8){
                    ...
                    nAnonymizeBitRewardsAmount = (rand() % nLeftToAnon)+3;
                } else {
